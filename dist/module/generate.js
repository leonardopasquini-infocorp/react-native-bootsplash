...
const getAndroidOutputPath = ({
  ...
  const androidOutputPath = path.resolve(sourceDir, fs.existsSync(appDir) ? "app" : "", "src", flavor, "res");
  if (hfs.exists(androidOutputPath)) {
    return androidOutputPath;
  }
  log.warn(`No ${path.relative(workingPath, androidOutputPath)} directory found. Skipping Android assets generation…`);
};
const getIOSOutputPath = () => {
  const podfile = glob.sync("**/Podfile", {
  ...
  const xcodeProjectName = fs.readdirSync(sourceDir).sort().reverse().find(fileName => {
  ...
  const iosOutputPath = path.resolve(sourceDir, xcodeProjectName).replace(/\.(xcodeproj|xcworkspace)$/, "");
  if (hfs.exists(iosOutputPath)) {
    return iosOutputPath;
  }
  log.warn(`No ${path.relative(workingPath, iosOutputPath)} directory found. Skipping iOS assets generation…`);
};
const getHtmlTemplatePath = ({
  ...
  log.warn(`No ${path.relative(workingPath, htmlTemplatePath)} found. Skipping HTML + CSS generation…`);
};
